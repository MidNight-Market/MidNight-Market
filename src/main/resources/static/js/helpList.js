...
for(let i=0; i<adminReply.length; i++){
    adminReply[i].addEventListener('click',(e)=>{
        ...
        let secretCheck = e.currentTarget
            .closest("[data-secretCheck]")
            .getAttribute("data-secretCheck")
        let replyCheck = e.currentTarget
            .closest("[data-replyCheck]")
            .getAttribute("data-replyCheck")
        console.log(secretCheck, replyCheck);
        if(sesName != writer && secretCheck== "Y"){ //@@@@@@@@2관리자 부분 추가 필
